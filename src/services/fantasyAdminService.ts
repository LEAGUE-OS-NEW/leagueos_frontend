...
export async function fetchFantasyPlayerCandidates(competition:string){return (await apiClient.get('/fantasy/players/candidates/',{params:{competition}})).data as FantasyPlayerCandidate[];}
...
export async function fetchFantasyFixtureCandidates(competition:string){return (await apiClient.get('/fantasy/gameweeks/fixture-candidates/',{params:{competition}})).data as FantasyFixture[];}
...
export async function fetchFantasyStatisticTypes(competition:string){const rows=(await apiClient.get(`/fantasy/competitions/${id(competition)}/statistic-types/`)).data as Array<{code:string}>;return rows.map(row=>row.code);}
...
export async function fetchAdminLeagueOverview(){return (await apiClient.get('/fantasy/leagues/admin-overview/')).data as FantasyLeagueOverview[];}
