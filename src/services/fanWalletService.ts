...
let latestAvailableBalanceUgx = parseUGX(WALLET_DETAILS.balance);
...
function mapLedgerEntry(entry: LedgerEntryRead): WalletTransaction {
  const isCredit = entry.credit_account === 'PROVIDER_PAYABLE';
  const amount = formatUGX(Number(entry.amount));
  return {
    id: entry.id,
    label: isCredit ? 'Wallet credit' : 'Wallet debit',
...
export async function fetchWalletDetails(): Promise<WalletDetails> {
  try {
    const [walletResponse, ledgerResponse] = await Promise.all([
      axiosInstance.get<WalletRead>('/wallets/UGX/'),
      axiosInstance.get<LedgerEntryRead[] | { results: LedgerEntryRead[] }>('/wallets/UGX/ledger/'),
    ]);
    const wallet = walletResponse.data;
    const ledgerEntries = Array.isArray(ledgerResponse.data) ? ledgerResponse.data : ledgerResponse.data.results;
    const transactions = ledgerEntries.map(mapLedgerEntry);

    latestAvailableBalanceUgx = Number(wallet.available_balance);
    return {
      balance: formatUGX(latestAvailableBalanceUgx),
      pendingWithdrawals: formatUGX(Number(wallet.reserved_balance)),
      transactions,
    };
  } catch {
...
export function getWalletAvailableBalanceUgx(): number {
  return latestAvailableBalanceUgx;
}
...
export async function submitWithdrawal(amountUgx: number, destination: string): Promise<void> {
  await axiosInstance.post('/wallets/withdrawals/', {
    amount: amountUgx,
    currency: 'UGX',
    destination: { label: destination || 'Mobile Money' },
  });
}
