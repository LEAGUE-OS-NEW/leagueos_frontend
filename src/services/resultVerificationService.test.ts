...
import {
  ...
  verifyResult,
  voidMarket,
} from './resultVerificationService';
...
describe('result verification aggregation', () => {
  ...
  it(
    'preserves the authoritative settlement block reason while settlement is pending',
    async () => {
        ...
        data: [
          {
            id: 'm1',
            question: 'Q?',
            workflow_state: 'SETTLEMENT_PENDING',
            outcomes: [],
            can_settle: false,
            settles_by: '2026-01-02T10:00:00Z',
            settlement_block_reason:
              'Settlement becomes available at 2026-01-02T10:00:00Z.',
          },
        ],
      });
  ...
  it(
    'publishes a provisional result without settling or overriding the backend dispute-window default',
    async () => {
      vi.mocked(apiClient.get).mockResolvedValue({
        data: [
          {
            id: 'm1',
            question: 'Q?',
            workflow_state: 'AWAITING_RESULT',
            outcomes: [
              {
                id: 'outcome-1',
                side: 'YES',
      ...
      await verifyResult('m1', {
        winningOutcomeId: 'YES',
        evidenceNote: 'Official source',
      });

      expect(apiClient.post).toHaveBeenCalledTimes(1);

      expect(apiClient.post).toHaveBeenCalledWith(
        '/market-admin/markets/m1/provisional-result/',
        expect.not.objectContaining({
          dispute_window_hours: expect.anything(),
        }),
      );
